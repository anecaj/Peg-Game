...
public class PegGameTri implements PegGame{
    Board board;
    GameState gameState;

    public PegGameTri(Board board){
        this.board = board;
        this.gameState = GameState.NOT_STARTED;
    }

    public PegGameTri(int height){
        this.board = new Board(height);
        this.gameState = GameState.NOT_STARTED;
    }

    public PegGameTri(Board board, GameState gameState){
        this.board = board;
    ...
    public Collection<Move> getPossibleMoves() {
        ...
        int[] rowMods = {0, 0, 2, 2, -2, -2, -2, 2};
        int[] colMods = {-2, 2, -2, 2, -2, 2, 0, 0};

        //Same as above, but these will be used to see if theres a peg inbetween
        int[] pegRowMods = {0, 0, 1, 1, -1, -1, -1, 1};
        int[] pegColMods = {-1, 1, -1, 1, -1, 1, 0, 0};
        ...
        for (int row = 0; row < board.getLength(); row++) {
            ...
            for (int col = 0; col < board.getRowLength(row); col++) {
                ...
                if (gameBoard[row][col].equals("-")) {
                    ...
                    for (int i = 0; i < 8; i++) {
                        ...
                        if ((newRow >= 0) && (newCol >= 0) && (newRow < board.getLength()) && (newCol < board.getRowLength(newRow))) {
...
/**
    ...
    public PegGame deepCopy() {
        PegGame deepCopy = new PegGameTri(new Board(board), gameState);
        return deepCopy;
    }
